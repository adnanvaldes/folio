...
def mock_session(session):
    ...
    with patch("folio.books.books._get_session") as mock:
...
def test_find_work(session, sample_work):
    ...
    # Should not find non-existent work
...
def test_validate_isbn():
    ...
    # Mock typer.confirm to always return True for this test
    with patch("typer.confirm", return_value=True):      
        # Invalid ISBN should return None when confirmed to continue
...
def test_validate_isbn_abort():
    ...
    # Mock typer.confirm to return False (don't continue with NULL ISBN)
...
def test_create_work_and_book(session):
    ...
    with patch("folio.books.books.console.print"):  # Suppress console output
    ...
    # Verify book was associated with work
...
def test_create_work_without_book(session):
    ...
    with patch("folio.books.books.console.print"):  # Suppress console output
        work = BookCommands._create_work_and_book(
            ...
            genre="science",
            is_read=False,
            add_book=True,  # True but no book info provided
            pages=None,
            format=None,
            isbn=None
        )
...
def test_add_command_new_work(mock_session):
    ...
    with patch("folio.books.books.console.print"), \
         patch("folio.books.books.BookCommands._find_work", return_value=None), \
         patch("folio.books.books.BookCommands._create_work_and_book") as mock_create:
        ...
        # Verify _create_work_and_book was called with correct args
...
def test_add_command_existing_work(mock_session, sample_work):
    ...
    with patch("folio.books.books.console.print"), \
         patch("folio.books.books.BookCommands._find_work", return_value=sample_work), \
         patch("typer.confirm", return_value=True), \
         patch("folio.books.books.BookCommands._add_book_to_work") as mock_add_book:
        ...
        # Verify _add_book_to_work was called with correct args
        ...
        # assert call_args["isbn"] == "9780987654321"
...
def test_add_command_existing_work_no_book(mock_session, sample_work):
    ...
    with patch("folio.books.books.console.print"), \
        ...
        # Should exit with success but not add any books
...
def test_add_book_command_existing_work(mock_session, sample_work):
    ...
    with patch("folio.books.books.console.print"), \
        ...
        # Should successfully add book
        assert result.exit_code == 0


def test_add_book_command_new_work(mock_session):
    """Test the add_book command when work doesn't exist."""
    # Mock to return None (work not found) then a new work when created
    new_work = Work(id=1, title="new work", author="new author")
    
    with patch("folio.books.books.console.print"), \
         patch("folio.books.books.BookCommands._find_work", return_value=None), \
         patch("typer.confirm", return_value=True), \
         patch("folio.books.books.BookCommands.add_work", return_value=new_work):
        
        result = runner.invoke(
            app, 
            ["add-book", "new work", "new author", "--pages", "200", 
             "--format", "print", "--isbn", "9781122334455"]
        )
...
def test_add_book_command_work_not_found_abort(mock_session):
    ...
    with patch("folio.books.books.console.print"), \
        ...
        # Should print that no changes were made
        assert result.exit_code == 0
